...
import { addChapterMember } from '@/services/chapters.service'
import { toast } from '@/store/toast.store'

interface AddChapterMemberModalProps {
  chapterId: string
  chapterName: string
...
export function AddChapterMemberModal({ chapterId, chapterName, existingMemberIds, open, onClose }: AddChapterMemberModalProps) {
  ...
  const addMutation = useMutation({
    mutationFn: (userId: string) => addChapterMember(chapterId, userId),
    onSuccess: () => {
